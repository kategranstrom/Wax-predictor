	...
	var timeStart = timeEnd - ndays*24*60*60
 	jQuery.ajax({
		...
		success: function(data) {
			...
			//since the data is time-ordered, the last item in the array will be the closest in time to right now
			//the last item will be used for a right-now wax recommendation
			//and also to write to the wax_now span in the html.
		   	var last_item = our_custom_data[our_custom_data.length - 1]
		   	var key = 1; //the default value
		   	if (last_item['humidity'] < 70) {
		   		key = 0
                  	}
		    	//using simple (no humidity variable) wax reccomendation: key = 4
            		var swix_wax_now = waxRecommender(last_item['temperature'], 4)
			//write the last wax recommendation to the "wax_now" span
             		document.getElementById("swix_wax_now").innerHTML = swix_wax_now;
			//simple (no humidity variable) rhode waxes: key = 5
		     	var rode_wax_now = waxRecommender(last_item['temperature'] , 5)
			document.getElementById("rode_wax_now").innerHTML = rode_wax_now;
			//adjust text colour if temerature is above zero
...
function waxRecommender(temp, key){
	//lookUpTable columns description:
